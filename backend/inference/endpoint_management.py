...
def deploy_endpoint(job_id:str,engine:str,instance_type:str,quantize:str,enable_lora:bool) -> Dict[bool,str]:
    ...
    if jobinfo.job_payload['finetuning_method'] == 'lora':
        model_s3_path = jobinfo.output_s3_path + 'finetuned_model_merged/'
    else:
        model_s3_path = jobinfo.output_s3_path + 'finetuned_model/'
    ...
    endpoint_name = sagemaker.utils.name_from_base(jobinfo.job_payload["model_name"]).replace('.','-').replace('_','-')
