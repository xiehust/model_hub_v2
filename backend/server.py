...
async def handle_deploy_endpoint(request:DeployModelRequest):
    logger.info(request)
    ret,endpoint_name = deploy_endpoint(job_id=request.job_id,engine=request.engine,instance_type=request.instance_type,quantize=request.quantize,enable_lora=request.enable_lora,model_name=request.model_name)
